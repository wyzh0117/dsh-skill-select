 ...
 *  - 不存在则回退：在右上角挂载"技能"开关，点击从右侧滑出抽屉面板；
  ...
  factory: (require) => {
    ...
    async function setDefaultStart(name, on) {
      try {
        ...
        setSkillState({
          skills: current.map((s) => (s.name === name ? { ...s, defaultStart: on } : s)),
        });
      } catch (error) {
    ...
    /** 由勾选名单推导草稿令牌：repo 满选 → 一个 `/repo`；否则逐个 `/skill`。 */
    function tokensForChecked(skills, checked) {
      ...
      const usable = repoTokens(skills);
      for (const [repo, members] of byRepo) {
        if (!usable.has(repo)) continue;
        if (members.length === 0) continue;
        if (members.every((m) => checkedSet.has(m.name))) {
          tokens.push(`/${repo}`);
          for (const m of members) remaining.delete(m.name);
        }
      }
      for (const name of checked) {
        if (remaining.has(name)) tokens.push(`/${name}`);
      }
      return tokens;
    }

    /** 从草稿移除本插件管理的 `/token` 手势（repo 令牌 ∪ 技能名），清理空白。 */
    function stripManagedTokens(draft, managed) {
      if (typeof draft !== "string") return "";
      return draft.replace(GESTURE_TOKEN_RE, (m, lead, token) => {
        if (managed.has(token)) return "";
        return m;
      }).replace(/\s{2,}/g, " ").trim();
    }
    ...
    function composeDraft(skills, checked, currentDraft) {
      const managed = new Set(skills.map((s) => s.name));
      for (const token of repoTokens(skills)) managed.add(token);
    ...
    function repoCheckState(skills, repo, checked) {
      ...
      const checkedCount = members.filter((m) => checked.includes(m.name)).length;
        ...
        if (sortMode === "source") {
          ...
          for (const group of groups) {
            ...
            const toggleAll = () => {
              ...
              applyCheckedBulk(rootCtx, sessionId, members.map((m) => ({ name: m.name, on: target })));
            };
            rows.push(React.createElement("div", {
              key: `repo-${repoKey}`,
              ...
              `${group.repo || "Other"} (${members.length})`));
        ...
        for (const skill of items) {
          rows.push(React.createElement("div", {
            ...
            React.createElement(Checkbox, {
              ...
              onChange: (on) => { setDefaultStart(skill.name, on); },
            })));
        }
      }
      ...
      return ReactDOM.createPortal(
        React.createElement(React.Fragment, null,
          React.createElement("button", {
            ...
            style: {
              position: "fixed", top: top + 6, right: 12, zIndex: 9200,
          ...
          React.createElement("div", {
            style: {
              position: "fixed", top: top, right: 0, bottom: 0, width: 380, maxWidth: "92vw",
            ...
            React.createElement("div", {
              style: {
                height: 36, padding: "0 48px 0 12px",
