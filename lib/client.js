...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const checkedBySession = new Map();
    ...
    const checkedFor = (sessionId) => {
      ...
      if (list === undefined) {
        ...
        checkedBySession.set(sessionId, list);
      }
      return list;
    };
    ...
    function SkillPanel({ rootCtx, sessionId, embedded }) {
      ...
      const checked = sessionId ? checkedMap.get(sessionId) ?? [] : [];

      React.useEffect(() => {
        if (sessionId && (state.sessionId !== sessionId || state.skills.length === 0)) {
          loadSkills(sessionId);
        }
      }, [sessionId, state.sessionId, state.skills.length]);
    ...
    function FooterButton({ rootCtx, useSessions }) {
      ...
      const listState = useSessions ? useSessions() : undefined;
    ...
    // ── 可选服务探测（三保险）───────────────────────────────────────────────
    function withBetterSidebar(ctx, use) {
      let done = false;
      const applyOnce = (sidebar) => {
        if (done || sidebar === undefined || sidebar === null) return;
        done = true;
        use(sidebar);
      };
      applyOnce(ctx.get("betterSidebar"));
      if (done) return;
    ...
    function apply(ctx) {
      let fallback = false;
      withBetterSidebar(ctx, (sidebar) => {
        registerTab(ctx, sidebar);
      });
      // 回退：3.2s 后仍未发现 betterSidebar 则注册内置侧栏 footer 按钮。
      setTimeout(() => {
