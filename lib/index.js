  ...
  async #onPreStep(decision, agent, messages) {
    ...
    if (hasUser && agent !== undefined && agent !== null && !this.#defaultInjected.has(agent)) {
      ...
      for (const name of this.#defaults) {
        ...
        try {
          const skill = await this.ctx.skills.get(name, view).catch(() => undefined);
    ...
    switch (method) {
      ...
      case "set-checked": {
        ...
        if (!Array.isArray(skills) || !skills.every((s) => typeof s === "string")) {
          throw new SkillSelectApiError("bad-request", "missing or invalid \"skills\"");
        }
