 ...
 *   - set-guard { on }                   → 开/关 skill 工具守卫（关闭即回到无本
 *                                          插件的默认工作流）
 *
...
export default class SkillSelectService extends Service {
  ...
  async [Service.init]() {
    ...
    this.#guard = domain.global.get().guard ?? false;
    this.#registerRoute();
  ...
  /**
   * skill 工具守卫：开关打开时，模型对「默认启动名单 ∪ 本会话勾选名单」之外的
   * 技能调用被拒绝；开关关闭时完全放行（回到无本插件的默认工作流）。
  ...
  async #dispatch(method, payload) {
    ...
    switch (method) {
      case "list": {
        ...
        return { ...result, guard: this.guard };
      }
      case "summarize": {
        const sessionId = requireString(payload, "sessionId");
      ...
      case "set-defaults": {
        ...
        const valid = Array.isArray(names) && names.every((n) => typeof n === "string" && n !== ""
          && (isSkillName(n) || /^[a-z0-9]+(?:-[a-z0-9]+)*:[a-z0-9]+(?:-[a-z0-9]+)*$/.test(n)));
        if (!valid) throw new SkillSelectApiError("bad-request", "missing or invalid \"names\"");
        if (typeof payload.on !== "boolean") {
          throw new SkillSelectApiError("bad-request", "missing or invalid \"on\"");
        }
        let next = this.#defaults;
        for (const name of names) next = toggleDefaults(next, name, payload.on);
        const state = this.#domain.global.get();
        await this.#domain.global.set({ ...state, defaults: next });
        this.#defaults = next;
      ...
      case "set-guard": {
        ...
        const state = this.#domain.global.get();
        await this.#domain.global.set({ ...state, guard: payload.on });
        this.#guard = payload.on;
        return { guard: payload.on };
      }
      case "set-checked": {
        const sessionId = requireString(payload, "sessionId");
